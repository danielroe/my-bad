...
> Beautiful dev-server error pages. One sourcemapped report, rendered as HTML, ANSI or JSON, kept live over SSE.

`my-bad` turns an error into a plain JSON `ErrorReport` (sourcemapped frames, snippets, causes, component trace, request context) and renders that report as a full HTML page, a shadow-DOM overlay for an existing page, or ANSI for the terminal. A small SSE channel keeps pages live: errors swap in place, pages reload when the error is fixed, warnings and server logs stream in.

If you are looking for a batteries-included error page today, [youch](https://github.com/poppinss/youch) has been solving the same problem for years and is what Nuxt used before this.
...
Inject the error UI into an existing page (for example a framework's rendered `error.vue`). It renders in a shadow root, so host styles cannot leak in, and can be minimised to a picture-in-picture thumbnail so the user can see the page behind it.
...
The page is dark by default and follows `prefers-color-scheme`. Everything is derived from a handful of CSS custom properties, so a framework can restyle it without forking the stylesheet:
...
- Run the accessibility audit (computed contrast, axe-core, target sizes, focus, motion, reflow) with `pnpm test:a11y`
- Render screenshots of the current UI with `pnpm screenshot`; drive the page and overlay over a live channel with `pnpm play`; try the Vite plugin (in-app compile error overlay and a server error page) with `pnpm play:vite`
